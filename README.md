# Hi, I'm Krish Jaiswal 👋

B.Tech CSE '29 | Learning DSA & Full Stack Dev | Passionate about building things 🚀

---

## 🛠️ Tech Stack

**Languages**
C • C++ • Python • JavaScript • HTML • CSS

**Backend**
Node.js • Express • Mongoose • MongoDB

**Tools & Others**
Git • GitHub • Figma

---

## 🚀 Projects

### 🌐 Personal Portfolio Website
My personal portfolio showcasing my projects and skills.

### 🎬 Movie Discovery App
A movie discovery app with pagination built using a third party API.

### 👨‍💼 Employee Generator
An Express + Mongoose app that generates dummy employee data and stores it in MongoDB.

---

## 📫 Connect with me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/krish-jaiswal-cse/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kajejaiswal@gmail.com)>
